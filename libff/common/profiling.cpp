                                            ...
                                            long long cpu_now, long long cpu_last)
{
    ...
    if (time_from_last != 0) {
        long long parallelism_from_last = cpu_time_from_last / time_from_last;
        printf("[%0.4fs x%0.2f]", (double) time_from_last * 1e-9, (double) parallelism_from_last);
    } else {
        printf("[             ]");
    }
    if (time_from_start != 0) {
        long long parallelism_from_start = cpu_time_from_start / time_from_start;
        printf("\t(%0.4fs x%0.2f from start)", (double) time_from_start * 1e-9, (double) parallelism_from_start);
    }
}
